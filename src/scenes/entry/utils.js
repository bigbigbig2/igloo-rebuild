...
export function ensureEntryFloorAttributes(geometry) {
  ...
  const parents = new Int32Array(vertexCount);
  const coincidentVertices = new Map();
  const animationMask = new Float32Array(vertexCount);
  const iteration = new Float32Array(vertexCount);
  ...
  for (let index = 0; index < vertexCount; index += 1) {
    const root = findRoot(parents, index);
    const centroid = componentCentroids.get(root) ?? new THREE.Vector3();
    const pieceSize = componentCounts.get(root) ?? 1;
    const radial = Math.sqrt(centroid.x * centroid.x + centroid.z * centroid.z);
    const mask = clamp01(1 - radial / 1.95);
    const sizeBias = clamp01((pieceSize - 6) / 42);

    vertex.fromBufferAttribute(position, index);
    animationMask[index] = THREE.MathUtils.lerp(mask * 0.4, mask, 0.6 + sizeBias * 0.4);
    iteration[index] = hashFloat(root * 0.73 + centroid.x * 2.1 + centroid.z * 1.7) * Math.PI * 2;
