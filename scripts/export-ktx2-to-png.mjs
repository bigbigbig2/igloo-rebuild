...
const outputRoot = path.resolve(projectRoot, 'recovered-assets', 'ktx2-png');
const exporterPagePath = path.resolve(projectRoot, 'tools', 'ktx2-exporter.html');
...
function shouldSkipEntry(entry) {
  ...
  if (entry.source.includes('/images/volumes/')) {
    return '3d-volume';
  }
...
function createTextureTasks() {
  return assetManifest.texture
    .filter((entry) => entry.source.toLowerCase().endsWith('.ktx2'))
    .map((entry) => ({
      key: entry.key,
      ...
      skipReason: shouldSkipEntry(entry),
...
function resolveRequestPath(urlPathname) {
  if (urlPathname === '/' || urlPathname === '/tools/ktx2-exporter.html') {
...
async function writeBase64Png(outputPath, dataUrl) {
...
async function main() {
  ...
  const tasks = createTextureTasks();
  const report = {
  ...
  try {
    ...
    await page.goto(`${origin}/tools/ktx2-exporter.html`, {
      waitUntil: 'domcontentloaded'
    });
    await page.waitForFunction(() => window.__ktxExporterReady === true);
    ...
    for (const task of tasks) {
      ...
      if (task.skipReason) {
        ...
        console.log(`[skip] ${task.key} -> ${task.skipReason}`);
        continue;
      }
      ...
      const result = await page.evaluate(async (pageTask) => {
        return await window.__exportKtx2ToPng(pageTask);
      }, task);

      if (result.status === 'exported') {
        await writeBase64Png(outputPath, result.dataUrl);
